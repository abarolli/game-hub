...
function App() {
	...
	const [currentPlatform, setCurrentPlatform] = useState<number | null>(null);
	...
	const appendGames = (games: RawgGame[]) => {
		const newGames: GameProps[] = [];
		for (let game of games) newGames.push(rawgGameToGameProps(game));
	...
	const loadMoreGames = useCallback(() => {
		...
		gamesHttpService
			.find({ page: page.current, genres: currentGenre, parent_platforms: currentPlatform })
			.then((games) => {
				appendGames(games);
				page.current += 1;
			});
	...
	useEffect(() => {
		...
		const observer = createObserver(sentinel);
	...
	const platformSelectHandler = (id: number) => {
		gamesHttpService
			.find({ genres: currentGenre, parent_platforms: id })
			.then((games) => {
				updateGames(games);
	...
	const genreClickHandler = (id: number) => {
		gamesHttpService
			.find({ genres: id, parent_platforms: currentPlatform })
			.then((games) => {
				updateGames(games);
				setCurrentGenre(id);
				page.current = 2;
	...
	const createObserver = (target: any): IntersectionObserver => {
			...
			<Flex justifyContent="center">
				<Flex maxW="1500px" w="100%">
					<Box w="300px" p="20px">
						...
						<GenreList selected={currentGenre} genres={genres} onClick={genreClickHandler} />
					</Box>
					<SimpleGrid
						minChildWidth="xs"
						columnGap="20px"
